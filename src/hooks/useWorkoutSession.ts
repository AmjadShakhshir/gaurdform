...
export interface WorkoutSessionState {
  ...
  markSetDone: (exerciseId: ExerciseId) => void;
  advanceToNext: () => void;
...
export function useWorkoutSession(): WorkoutSessionState {
  ...
  const todayWorkout: WorkoutDay | null = plan
  ...
  const isWorkoutComplete =
    ...
    todayWorkout.exercises.every(
      (ex) => (completedSets[ex.exerciseId] ?? 0) >= ex.sets,
    );

  // Auto-advance currentExerciseIdx in guided mode when a set is marked done
  const prevSetsRef = useRef<Record<string, number>>({});
  useEffect(() => {
  ...
  const markSetDone = useCallback((exerciseId: ExerciseId) => {
  ...
  const advanceToNext = useCallback(() => {
    ...
    setCurrentExerciseIdx((idx) =>
      Math.min(idx + 1, todayWorkout.exercises.length - 1),
    );
  }, [todayWorkout]);
  ...
  const completeWorkout = useCallback(async () => {
    ...
    if (plan.lastWorkoutDate === today) return;
