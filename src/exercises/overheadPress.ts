...
export const overheadPress: ExerciseDefinition = {
  id: "overheadPress",
  ...
  evaluate(
    ...
    if (state.phase === "READY" && avgElbow < ELBOW_START - HYSTERESIS) {
      ...
      if (avgElbow < state.scratch.minElbow) state.scratch.minElbow = avgElbow;
      if (backLean > state.scratch.maxBackLean) state.scratch.maxBackLean = backLean;
      if (avgElbow >= ELBOW_TOP + HYSTERESIS) state.phase = "TOP";
