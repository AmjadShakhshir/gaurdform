...
function repQuality(minKnee, maxBackLean, symmetryDelta, heelRose, buttWink) {
    ...
    return Math.round(0.40 * depth + 0.25 * lean + 0.20 * sym + 0.10 * heel + 0.05 * wink);
}
// ── Exercise definition ────────────────────────────────────────────────────────────────
export const squat = {
    id: "squat",
    ...
    setupTip: "Stand ~2 m from camera. Face it for knee-valgus feedback, or stand sideways for depth & heel-rise checks. Full body must be in frame.",
    initialState() {
    ...
    evaluate(landmarks, prev, nowMs, viewAngle) {
        ...
        if (state.phase === "READY" && kneeAng < DESC_KNEE - HYSTERESIS) {
            ...
            if (kneeAng > TOP_KNEE + HYSTERESIS) {
                ...
                const score = repQuality(state.scratch.minKnee, state.scratch.maxBackLean, state.scratch.lastBottomSymmetry, heelRose, buttWink);
                state.repLog = [
                    ...state.repLog,
                    {
                        repIndex: state.reps,
        ...
        if (!isSide) {
            ...
            if (state.scratch.baselineAnkleYL > 0 && inRep) {
                ...
                if (maxRise > state.scratch.heelRose)
                    state.scratch.heelRose = maxRise;
            ...
            if (state.scratch.baselineWristMidX > 0 && inRep && visible([lWrist, rWrist], 0.25)) {
                ...
                if (dev > state.scratch.maxBarPathDev)
                    state.scratch.maxBarPathDev = dev;
