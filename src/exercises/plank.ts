...
function holdScore(holdSec: number, avgBodyDeviation: number, maxSagDeg: number): number {
...
export const plank: ExerciseDefinition = {
  ...
  setupTip:
    "Set camera to your side. Hold a straight shoulder-hip-ankle line and keep your full body in frame.",
  ...
  evaluate(
    ...
    if (state.phase === "HOLDING") {
      ...
      if (bodyLine < MAX_HIP_SAG_ANGLE) {
        ...
        if (sag > state.scratch.maxSagDeg) state.scratch.maxSagDeg = sag;
        feedback.push({
          id: "hip-sag",
          severity: "error",
      ...
      if (holdSec >= HOLD_TARGET_SEC) {
        const avgBodyDeviation =
          state.scratch.alignmentDeviationSum / Math.max(1, state.scratch.alignmentFrames);
