...
export const rdl: ExerciseDefinition = {
  ...
  evaluate(
    ...
    if (state.phase === "READY" && torsoAng > HINGE_START + HYSTERESIS) {
      ...
      if (torsoAng > state.scratch.maxHinge) state.scratch.maxHinge = torsoAng;
      if (torsoAng >= HINGE_BOTTOM + HYSTERESIS) state.phase = "BOTTOM";
    } else if (state.phase === "BOTTOM") {
      if (torsoAng > state.scratch.maxHinge) state.scratch.maxHinge = torsoAng;
      if (torsoAng < HINGE_ASCEND - HYSTERESIS) state.phase = "ASCENDING";
