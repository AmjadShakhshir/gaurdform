...
function repQuality(minElbow, tempoSec, maxBodyDeviation, maxHeadLift) {
...
export const pushUp = {
    ...
    setupTip: "Set the camera to your side. Keep your full body visible and hold a high-plank shape for 2 s.",
    initialState() {
        elbowSmoother.reset();
    ...
    evaluate(landmarks, prev, nowMs, viewAngle) {
        ...
        if (state.phase === "READY" && elbowAngle < DESC_ELBOW - HYSTERESIS) {
            ...
            if (elbowAngle < state.scratch.minElbow)
                state.scratch.minElbow = elbowAngle;
            if (bodyDeviation > state.scratch.maxBodyDeviation) {
                state.scratch.maxBodyDeviation = bodyDeviation;
            }
            if (headLift > state.scratch.maxHeadLift)
                state.scratch.maxHeadLift = headLift;
            if (elbowAngle <= BOTTOM_ELBOW - HYSTERESIS)
                state.phase = "BOTTOM";
        }
            ...
            if (headLift > state.scratch.maxHeadLift)
                state.scratch.maxHeadLift = headLift;
            if (elbowAngle > ASCEND_ELBOW + HYSTERESIS)
            ...
            if (elbowAngle >= TOP_ELBOW + HYSTERESIS) {
                ...
                const score = repQuality(state.scratch.minElbow, tempoSec, state.scratch.maxBodyDeviation, state.scratch.maxHeadLift);
                state.repLog = [
                    ...state.repLog,
                    {
                        repIndex: state.reps,
