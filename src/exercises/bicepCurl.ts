...
export const bicepCurl: ExerciseDefinition = {
  ...
  evaluate(landmarks: Landmarks, prev: ExerciseState, nowMs: number, _viewAngle: ViewAngle): EvalResult {
    ...
    const sideLM = SIDE_LM[side as 0 | 1];
    ...
    if (state.phase === "READY" && elbowAng < EXTENDED - HYSTERESIS) {
      ...
      if (elbowAng < CONTRACTED - HYSTERESIS) state.phase = "BOTTOM";
    } else if (state.phase === "BOTTOM") {
      if (elbowAng < state.scratch.minElbow) state.scratch.minElbow = elbowAng;
      if (elbowAng > DESC_FROM_TOP + HYSTERESIS) state.phase = "ASCENDING";
      ...
      if (elbowAng > EXTENDED + HYSTERESIS) {
        if (state.scratch.minElbow <= CONTRACTED + 5) {
          ...
          const score = repQuality(
            ...
            tempoSec,
            state.scratch.maxElbowDriftX,
            state.scratch.maxShoulderHikeY,
          );
    ...
    if (state.scratch.restingElbowX >= 0 && state.phase !== "READY") {
      ...
      if (drift > state.scratch.maxElbowDriftX) state.scratch.maxElbowDriftX = drift;
    ...
    if (
      state.scratch.restingShoulderY >= 0 &&
      state.scratch.torsoLength > 0 &&
      state.phase !== "READY"
    ) {
      const hikeNorm =
        (state.scratch.restingShoulderY - smoothedShoulderY) / state.scratch.torsoLength;
      if (hikeNorm > state.scratch.maxShoulderHikeY) state.scratch.maxShoulderHikeY = hikeNorm;
