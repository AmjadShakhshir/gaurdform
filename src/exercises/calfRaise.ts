...
export const calfRaise: ExerciseDefinition = {
  id: "calfRaise",
  label: "Calf Raise",
  icon: "👟",
  category: "Legs",
  ...
  evaluate(
    ...
    if (state.phase === "READY" && rise > RISE_THRESH + HYSTERESIS) {
      ...
      if (rise > state.scratch.maxRise) state.scratch.maxRise = rise;
      if (rise >= PEAK_THRESH + HYSTERESIS) state.phase = "TOP";
    } else if (state.phase === "TOP") {
      if (rise > state.scratch.maxRise) state.scratch.maxRise = rise;
      if (rise < PEAK_THRESH - HYSTERESIS) state.phase = "DESCENDING";
