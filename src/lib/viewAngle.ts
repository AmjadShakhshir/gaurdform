import { LM, type Landmarks } from "./pose";

/** Detected camera orientation relative to the trainee. */
export type ViewAngle = "front" | "side-left" | "side-right" | "back" | "unknown";

// Rolling mode-vote buffer — 7 frames smooths out momentary mis-detections.
const VIEW_WINDOW = 7;
const viewBuffer: ViewAngle[] = [];
 ...
 */
function rawDetect(landmarks: Landmarks): ViewAngle {
  ...
  if (lShVis < 0.20 && rShVis < 0.20) return "unknown";
...
function modeOf(arr: ViewAngle[]): ViewAngle {
  const counts = new Map<ViewAngle, number>();
  for (const v of arr) counts.set(v, (counts.get(v) ?? 0) + 1);
  let best: ViewAngle = "unknown";
  let bestCount = 0;
 ...
 */
export function detectViewAngle(landmarks: Landmarks): ViewAngle {
  const raw = rawDetect(landmarks);
  viewBuffer.push(raw);
  if (viewBuffer.length > VIEW_WINDOW) viewBuffer.shift();
  return modeOf(viewBuffer);
}

/** Human-readable label for the view angle badge. */
export function viewAngleLabel(v: ViewAngle): string {
  switch (v) {
