...
import type { LocalSession, PersonalRecord } from "./localDb";
import { getPR, savePR } from "./localDb";

/**
 * Maps each exercise to the metrics worth tracking as PRs.
 * "higher is better" for all — depth metrics are inverted (smaller angle = deeper = better),
 * so we store them as (180 - value) to maintain "higher is better" invariant.
 */
export interface PRMetricDef {
  metric: string;       // key in RepSummary.metrics or synthetic ("mostReps", "avgScore")
  label: string;        // human-readable name
  unit: string;         // display unit
  /** If true, the raw value is inverted (lower raw = better). Display shows raw value. */
  invertForComparison?: boolean;
  /** If true, this is a session-level metric (not per-rep). */
  sessionLevel?: boolean;
}

const EXERCISE_PR_METRICS: Record<ExerciseId, PRMetricDef[]> = {
  squat: [
...
const UNIVERSAL_SESSION_PRS: PRMetricDef[] = [
...
export async function detectAndSavePRs(
  ...
  for (const def of repMetrics) {
    if (!session.metrics_json?.length) continue;
