...
export interface ProgressSummary {
  ...
  totalReps: number;
  trendPercent: number | null; // null if insufficient data
...
export function getProgressData(
  ...
  // Group by day
  const byDay = new Map<string, { scores: number[]; reps: number }>();
  for (const s of relevant) {
    const date = toLocalDate(s.created_at!);
    if (!byDay.has(date)) byDay.set(date, { scores: [], reps: 0 });
    const entry = byDay.get(date)!;
    if (s.avg_metric != null && s.avg_metric > 0) entry.scores.push(s.avg_metric);
    entry.reps += s.reps;
  }

  // Build sorted points
  const points: ProgressPoint[] = Array.from(byDay.entries())
